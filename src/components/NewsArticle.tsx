...
export default function NewsArticle({ article }: { article: NewsItem }) {
  ...
  return (
    <article className="break-inside-avoid mb-6 group relative overflow-hidden rounded-2xl border bg-white shadow-sm transition hover:shadow-lg hover:border-zinc-200 focus-within:ring-2 focus-within:ring-black/10">
      ...
      {article.image && (
        <div className="relative h-44 w-full overflow-hidden bg-zinc-100">
          ...
          {article.image.startsWith("/sources/") && (
            <span className="news-item-image-source">{article.sourceName}</span>
          )}
        </div>
      )}
